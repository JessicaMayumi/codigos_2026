...
@router.get("/{equipamento_id}", response_model=EquipamentoOut, summary="Busca um equipamento pelo id",
            responses={404: {"description": "Equipamento nao encontrado"}})
...
@router.post("", response_model=EquipamentoOut, status_code=status.HTTP_201_CREATED,
             ...
             responses={400: {"description": "Tipo de equipamento invalido"},
...
@router.put("/{equipamento_id}", response_model=EquipamentoOut, summary="Atualiza um equipamento",
            description="Aceita atualizacao parcial: envie somente os campos que deseja alterar.",
            responses={400: {"description": "Tipo de equipamento invalido"},
                       404: {"description": "Equipamento nao encontrado"}})
...
@router.delete("/{equipamento_id}", status_code=status.HTTP_200_OK,
               summary="Remove um equipamento (requer autenticacao)",
               description="Rota protegida: envie o token JWT no header `Authorization: Bearer <token>`.",
               responses={401: {"description": "Token ausente, invalido ou expirado"},
                          404: {"description": "Equipamento nao encontrado"}})
